...
| **ADR-018** | Patrón Adapter para Almacenamiento de Archivos (`StorageAdapter`) | Aceptada | Desacopla la lógica de subida del mecanismo físico de persistencia (Local / Supabase), permitiendo cambiar de proveedor mediante configuración (`STORAGE_METHOD`) sin modificar código cliente. |
| **ADR-019** | Uso de SDK Oficial `@supabase/supabase-js` para Supabase Storage | Aceptada | Provee tipado nativo, cliente oficial mantenido, manejo automático de headers de autenticación y soporte para `getPublicUrl()`. |
| **ADR-020** | Supabase Client Provider con Fallback Seguro (Null-Safe Factory) | Aceptada | Si `SUPABASE_URL` o `SUPABASE_SERVICE_ROLE_KEY` no están configuradas, el provider retorna `null` con un warning en lugar de crashear la aplicación al inicio. Permite ejecutar en modo local sin credenciales de Supabase. |
| **ADR-021** | Generación de Nombres Únicos con `randomUUID()` + Extensión Original | Aceptada | Previene colisiones de nombres y sobreescrituras accidentales en disco o bucket, manteniendo la extensión original para correcta inferencia de MIME type. |
| **ADR-022** | Contrato `StorageAdapter` con `delete()` como `NotImplementedException` | Aceptada | Define la interfaz completa para el ciclo de vida de archivos a futuro sin incurrir en deuda técnica, manteniendo la regla de negocio del MVP de no implementar eliminación en esta fase. |
| **ADR-023** | Separación de Módulos `StorageModule` y `UploaderModule` | Aceptada | `StorageModule` encapsula adapters y servicios de bajo nivel (infraestructura); `UploaderModule` expone el orquestador de alto nivel (`UploadFileService`) como único punto de consumo para módulos de negocio. |
| **ADR-024** | Transformación Interna de `Express.Multer.File` a `UploadFileDTO` | Aceptada | `UploadFileService.execute()` recibe el archivo de Multer directamente y lo mapea internamente al DTO tipado, desacoplando los adapters de la dependencia directa con Multer/Express. |
| **ADR-025** | Mocking de `fs` en Tests E2E de Local Storage (Sin I/O Real a Disco) | Aceptada | Previene la creación de archivos residuales en disco durante la ejecución de la suite de pruebas e2e y garantiza portabilidad en entornos de CI/CD efímeros. |
